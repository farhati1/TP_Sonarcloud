    ...
    private class NanoServer extends NanoHTTPD {
        ...
        public Response serve(IHTTPSession session) {
            ...
            if (audioFileToServe == null) {
                ...
                return newFixedLengthResponse(Response.Status.NOT_FOUND, CONTENT_TYPE_HTML, "File not found");
            }
            ...
            if (uri.contains("audio")) {
                try {
                    ...
                    if (start <= end) {
                        ...
                        Response response = newFixedLengthResponse(Response.Status.PARTIAL_CONTENT,
                                getMimeType(audioFileToServe), audioInputStream, contentLength);
                        response.addHeader("Content-Length", contentLength + "");
                        response.addHeader("Content-Range", "bytes " + start + "-" + end + "/" + fileLength);
                        response.addHeader("Content-Type", getMimeType(audioFileToServe));
                        return response;
                    } else {
                        return newFixedLengthResponse(Response.Status.RANGE_NOT_SATISFIABLE, CONTENT_TYPE_HTML, range);
                    }
                } catch (IOException e) {
                ...
                if (imageBytesToServe == null) {
                    return newFixedLengthResponse(Response.Status.NOT_FOUND, CONTENT_TYPE_HTML, "Image bytes null");
                }
                cleanupImageStream();
                imageInputStream = new ByteArrayInputStream(imageBytesToServe);
                Log.i(TAG, "Serving image bytes: " + imageBytesToServe.length);
                return newFixedLengthResponse(Response.Status.OK, "image/png", imageInputStream,
                        imageBytesToServe.length);
            }
